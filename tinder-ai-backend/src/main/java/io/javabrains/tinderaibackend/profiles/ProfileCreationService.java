...
public class ProfileCreationService {
    ...
    public ProfileCreationService(OllamaChatClient chatClient, ProfileRepository profileRepository) {
    ...
    public void createProfiles(int numberOfProfiles) {
        ...
        while (this.generatedProfiles.size() < numberOfProfiles) {
            ...
            Prompt promptObj = new Prompt(prompt);  // Simple prompt text
            ChatResponse response = chatClient.call(promptObj);  // Already-configured OllamaChatClient will handle the function call
    ...
    private void saveProfilesToJson(List<Profile> generatedProfiles) {
        try {
            ...
            if (!file.exists()) {
                file.createNewFile();
                // Initialize with empty list to avoid null on reading
    ...
    public void saveProfilesToDB() {
        ...
        File file = new File(PROFILES_FILE_PATH);
        if (!file.exists()) {
            System.err.println("No profiles.json found. Skipping DB save.");
            return;
        }
